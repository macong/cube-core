...
var Mbox = {
	presets: {
		...
		ajaxAttr: "href",
		ajaxOptions: {},
		evalScriptsFlag:false,

		title: '提示框',
		ensure: '确定',
		cancel: '取消',
		clostxt: '×',
		loadimg: '../images/o_loading.gif',
	...
	loading: function(message,eventFlag) {
		this.initialize({loadimg:this.options.loadimg});
		message = message || '<div class="mbox_loading"><img src="'+this.options.loadimg+'" class="mbox_loading_image" />加载中...</div>';
		this.getContent(message,eventFlag);
		this.closbtn.setStyle("visibility", "hidden");
		this.bar.setStyle("display", "none");
		this.opt.setStyle("display", "none");
		return this;
	...
	close: function() {
		...
		if (this.isOpen) {
			...
			if ($type(this.options.onClose) === "function") {
				val = this.options.onClose.call(this);
			...
			//内容清空
			$$("iframe.mbox_ajax_iframe").set("src","");
			this.cont.empty();
			this.opt.empty();
	...
	toggleListeners: function(state) {
		...
		this.closbtn.removeEvents();//这个是很有必要添加的 不然事件没法清除，但不知道为什么。
	...
	getContent: function(content,eventFlag) {
		if(!eventFlag){
			...
			switch (type) {
				...
				case 'ajax': {
					new Request.HTML($merge({
						method: 'get'
					}, $extend({ evalScripts: this.options.evalScriptsFlag }, this.options.ajaxOptions))).addEvents({
