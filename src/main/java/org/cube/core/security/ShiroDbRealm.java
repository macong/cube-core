...
import org.cube.core.common.utils.Encodes;
import org.cube.core.security.utils.Digests;
import org.cube.core.uom.model.Role;
import org.cube.core.uom.model.User;
import org.cube.core.uom.service.AccountManager;
...
public class ShiroDbRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
		...
		if (user != null) {
			if (user.getUserStatus() == 0) {// 用户被禁用
				throw new DisabledAccountException();
			}

			byte[] salt = Encodes.decodeHex(user.getSalt());
			return new SimpleAuthenticationInfo(new ShiroUser(user.getUserName(), user.getRealName()), user.getUserPassword(), ByteSource.Util.bytes(salt), getName());
		} else {
			return null;
		}
	}
